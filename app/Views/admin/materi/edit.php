...
<body class="bg-burlap-50 font-sans min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden mb-8">
            <div class="md:flex">
                <div class="p-8">
                    ...
                    <p class="mt-3 text-stone-600">Perbarui informasi materi pembelajaran pada platform SekulLegend.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <div class="p-6">
                <form action="<?= base_url('admin/materi/update/' . $materi['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-6">
                    <div>
                        <label for="judul" class="block text-sm font-medium text-stone-700 mb-1">Judul Materi</label>
                        <input type="text" name="judul" id="judul" value="<?= esc($materi['judul']) ?>" required 
                            class="w-full rounded-md border-burlap-300 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-200 focus:ring-opacity-50 py-2 px-3 bg-amber-100">
                    </div>
                    
                    <div>
                        <label for="deskripsi" class="block text-sm font-medium text-stone-700 mb-1">Deskripsi Materi</label>
                        <textarea name="deskripsi" id="deskripsi" rows="6" 
                            class="w-full rounded-md border-burlap-300 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-200 focus:ring-opacity-50 py-2 px-3 bg-amber-100"><?= esc($materi['deskripsi']) ?></textarea>
                    </div>
                    
                    <div>
                        <label for="file" class="block text-sm font-medium text-stone-700 mb-1">File Materi (Kosongkan jika tidak diganti)</label>
                        <input type="file" name="file" id="file" 
                            class="w-full border border-burlap-300 rounded-md shadow-sm py-2 px-3 text-stone-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                        <?php if (!empty($materi['file'])): ?>
                        <p class="mt-2 text-sm text-stone-600 flex items-center">
                            ...
                            File saat ini: 
                            <a href="<?= base_url('public/uploads/materi/' . $materi['file']) ?>" target="_blank" class="ml-1 text-amber-700 hover:text-amber-800 hover:underline">
                                <?= esc($materi['file']) ?>
                            </a>
                        </p>
                        <?php endif; ?>
                    ...
                    <div class="flex items-center space-x-3 pt-3">
                        <button type="submit" class="px-4 py-2 bg-amber-600 text-white rounded-md hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 transition duration-150 ease-in-out">
                            <div class="flex items-center">
                        ...
                        <a href="<?= base_url('admin/materi') ?>" class="px-4 py-2 bg-stone-100 text-stone-700 rounded-md hover:bg-stone-200 focus:outline-none focus:ring-2 focus:ring-stone-500 focus:ring-offset-2 transition duration-150 ease-in-out">
                            Batal
                        </a>
                    </div>
                </form>
