...
$guru = isset($guru) && is_array($guru) ? $guru : [];
...
<head>
  ...
  <title>Tambah Kelas - SekulLegend</title>
      ...
      <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section class="mb-8 overflow-hidden rounded-2xl border border-amber-100 bg-white/90 p-6 shadow-xl backdrop-blur-sm sm:p-8">
          ...
          <p class="mt-4 max-w-2xl text-sm leading-6 text-stone-600 sm:text-base">
            Tambahkan kelas baru dan tentukan guru pengajar untuk kelas tersebut.
          </p>
        </section>
          ...
          <div class="overflow-hidden rounded-2xl border border-burlap-200 bg-white/90 shadow-md backdrop-blur-sm">
            <div class="border-b border-burlap-200 bg-burlap-50 px-6 py-4">
              ...
              <p class="mt-1 text-sm text-stone-500">
                Kode kelas boleh dikosongkan jika ingin dibuat otomatis oleh sistem.
              </p>
            </div>

            <div class="p-6">
              <form action="<?= base_url('admin/kelas/simpan') ?>" method="post" class="space-y-5">
                <?= csrf_field() ?>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Contoh: Bahasa Indonesia Kelas X"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Contoh: BIND10"
                    class="w-full rounded-lg border border-burlap-200 bg-burlap-50/50 px-4 py-3 text-sm uppercase tracking-wide text-stone-800 outline-none transition placeholder:normal-case placeholder:tracking-normal placeholder:text-stone-400 focus:border-amber-500 focus:ring-2 focus:ring-amber-200"
                  >

                  <p class="mt-2 text-xs text-burlap-600">
                    Kosongkan jika kode kelas ingin dibuat otomatis.
                  </p>
                </div>

                <div>
                  <label for="id_guru" class="mb-2 block text-sm font-medium text-stone-700">
                    Guru Pengajar
                  </label>
                  ...
                  >
                    <option value="" disabled <?= old('id_guru') ? '' : 'selected' ?>>
                      Pilih guru pengajar
                    </option>

                    <?php foreach ($guru as $g) : ?>
                      <option value="<?= esc($g['id'] ?? '') ?>" <?= (string) old('id_guru') === (string) ($g['id'] ?? '') ? 'selected' : '' ?>>
                        <?= esc($g['nama'] ?? '-') ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                  ...
                  <?php if (empty($guru)) : ?>
                    <p class="mt-2 text-xs text-red-600">
                      Belum ada data guru. Tambahkan user dengan role guru terlebih dahulu.
                    </p>
                  <?php endif; ?>
                </div>
                ...
                <div class="flex flex-col gap-3 border-t border-burlap-200 pt-5 sm:flex-row sm:justify-end">
                  ...
                  <button
                    type="submit"
                    class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-amber-600 px-5 py-3 text-sm font-semibold text-white shadow-md transition hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
                    <?= empty($guru) ? 'disabled' : '' ?>
                  >
                    <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
