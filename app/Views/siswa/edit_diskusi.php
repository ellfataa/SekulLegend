...
<body class="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-stone-200 font-sans text-stone-800">
  <main class="flex min-h-screen items-center justify-center px-4 py-8">
    <section class="w-full max-w-2xl">
      ...
      <div class="mb-5">
        <a
          href="<?= base_url('/siswa/kelas/' . ($komentar['id_kelas'] ?? '')) ?>"
      ...
      <div class="overflow-hidden rounded-2xl border border-burlap-200 bg-white/90 shadow-xl backdrop-blur-sm">
        <div class="border-b border-burlap-200 bg-burlap-50 px-6 py-5">
          ...
          <p class="mt-2 text-sm text-stone-500">
            Perbarui komentar diskusi Anda dengan bahasa yang jelas dan sopan.
          </p>
        </div>
        ...
        <div class="p-6">
          ...
          <form action="<?= base_url('/siswa/update-diskusi/' . ($komentar['id'] ?? '')) ?>" method="post" class="space-y-5">
            <?= csrf_field() ?>
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
              <button
                type="submit"
                class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-burlap-600 px-5 py-3 text-sm font-semibold text-white shadow-md transition hover:bg-burlap-700 focus:outline-none focus:ring-2 focus:ring-burlap-500 focus:ring-offset-2 sm:w-auto"
              >
                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
